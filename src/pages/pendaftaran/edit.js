...
function PendaftaranEdit() {
  ...
  const fetchOnePendaftaran = async () => {
    const res = await getData(`/cms/pendaftaran/${pendaftaranId}`);
    setForm({
      ...form,
  ...
  const handleSubmit = async () => {
    ...
    const res = await putData(`/cms/pendaftaran/${pendaftaranId}`, payload);
    if (res?.data?.data) {
      dispatch(
        setNotif(
          true,
  ...
  return (
    <Container>
      <SBreadCrumb
        textSecound={'Pendaftaran'}
        urlSecound={'/pendaftaran'}
        textThird='Edit'
      />
